...
proc coefficientsSGD(A: [?D] real, lRate: real, nEpoch: int){
  ...
  forall e in 1..nEpoch with (+ reduce sum_error, ref error, ref yhat){
    sum_error = 0.0;
    for r in 1..row{
...
proc rmse_metric(A: [?DA] real, B: [?DB] real): real{
  ...
  forall i in 1..A.size with (+ reduce sum_error, ref prediction_error) {
...
proc linearRegressionSGD(A: [?DA] real, B: [?DB] real, lRate: real, nEpoch: int) {
  ...
  forall i in 1..rows with (ref yhat) {
...
proc predict(A: [?DA] real, C: [?DC] real): real{
  ...
  forall i in 1..A.size-1 with (+ reduce yhat){
    yhat += C[i+1] * A[i];
  }
  return yhat;
}
